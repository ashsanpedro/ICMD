...
export class ListPanelPageComponent extends FormBaseComponent<SearchProjectFilterModel> {
    ...
    ngAfterViewInit(): void {
        ...
        this.panelTable.columnsChanged.subscribe(() => {
    ...
    //#region Delete Bulk
    protected async deleteBulkPanel(ids: string[]): Promise<void> {
        ...
        dialogRef.afterClosed().subscribe((result: string[] | null) => {
            if (result) {
                this._panelService.deleteBulkPanel(result).pipe(takeUntil(this._destroy$)).subscribe(
                    (res) => {
                        if (res && res.isSucceeded) {
                            this._toastr.success(res.message);
                            this.getPanelData();
    ...
    private tableColumnchanges() {
        this._cd.detectChanges();
        this.panelTable.columnFiltersList.forEach((filterComponent) => {
