...
export class ListStreamPageComponent {
    ...
    ngAfterViewInit(): void {
        ...
        this.streamTable.columnsChanged.subscribe(() => {
    ...
    //#region Delete Bulk
    protected async deleteBulkRoom(ids: string[]): Promise<void> {
        const dialogRef = this.dialog.open(TFRoomBulkDialogComponent, {
            width: "600px",
            data: ids
        });
        ...
        dialogRef.afterClosed().subscribe((result: string[] | null) => {
            if (result) {
                this._streamService.deleteBulkStream(result).pipe(takeUntil(this._destroy$)).subscribe(
                    (res) => {
                        if (res && res.isSucceeded) {
                            this._toastr.success(res.message);
                            this.getStreamData();
    ...
    private tableColumnchanges() {
        this._cd.detectChanges();
        this.streamTable.columnFiltersList.forEach((filterComponent) => {
    ...
    protected onFileSelected(event: any): void {
        ...
        this._streamService.validateImportStream(this.projectId, selectedFile)
            .pipe(takeUntil(this._destroy$))
            .subscribe({
                next: (res) => {
                    if (!res || !res.isSucceeded) {
    ...
    private proceedImport(selectedFile: File): void {
        ...
        this._streamService.importStream(this.projectId, selectedFile)
            .pipe(takeUntil(this._destroy$))
            .subscribe({
                next: (res) => {
                    if (res && res.isSucceeded) {
                        ...
                        if (res.records && res.records?.length > 0) 
                            this._excelHelper.downloadImportResponseFile<StreamInfoDtoModel>("Stream", res.records, importTagField3Columns);
                    } else {
                        this._toastr.error(res.message);
                    }
                    this.clearFileInput();
