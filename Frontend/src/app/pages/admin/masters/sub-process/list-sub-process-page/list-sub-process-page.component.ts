...
export class ListSubProcessPageComponent {
    ...
    ngAfterViewInit(): void {
        ...
        this.subProcessTable.columnsChanged.subscribe(() => {
    ...
    protected async deleteBulkSub(ids: string[]): Promise<void> {
        const dialogRef = this.dialog.open(TFSubLocBulkDialogComponent, {
            width: "600px",
            data: ids
        });
        ...
        dialogRef.afterClosed().subscribe((result: string[] | null) => {
            if (result) {
                this._subProcessService.deleteBulkSubProcess(result).pipe(takeUntil(this._destroy$)).subscribe(
                    (res) => {
                        if (res && res.isSucceeded) {
                            this._toastr.success(res.message);
                            this.getSubProcessData();
    ...
    private tableColumnchanges() {
        this._cd.detectChanges();
        this.subProcessTable.columnFiltersList.forEach((filterComponent) => {
    ...
    protected onFileSelected(event: any): void {
        ...
        this._subProcessService.validateImportSubProcess(this.projectId, selectedFile)
            .pipe(takeUntil(this._destroy$))
            .subscribe({
                next: (res) => {
                    if (!res || !res.isSucceeded) {
    ...
    private proceedImport(selectedFile: File): void {
        ...
        this._subProcessService.importSubProcess(this.projectId, selectedFile)
            .pipe(takeUntil(this._destroy$))
            .subscribe({
                next: (res) => {
                    if (res && res.isSucceeded) {
                        ...
                        if (res.records && res.records?.length > 0) 
                            this._excelHelper.downloadImportResponseFile<SubProcessInfoDtoModel>("Sub Process", res.records, importTagField2Columns);
                    } else {
                        this._toastr.error(res.message);
                    }
                    this.clearFileInput();
