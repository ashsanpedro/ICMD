...
import { JunctionBoxListDtoModel } from "./list-junction-box-table.model";
import { PermissionWrapperComponent } from "@c/shared/permission-wrapper";
import { AppConfig } from "src/app/app.config";
import { masterJunctionBoxListTableColumn } from "@u/constants";
...
@Component({
    standalone: true,
    selector: "app-list-junction-box-table",
    templateUrl: "./list-junction-box-table.component.html",
    imports: [
        FormDefaultsModule,
        MatTableModule,
        MatCheckbox,
...
export class ListJunctionBoxTableComponent implements OnInit {
    ...
    @Input() dataSource: MatTableDataSource<JunctionBoxListDtoModel>;
    @Input() totalLength: number = 0;
    selection = new SelectionModel<JunctionBoxListDtoModel>(true, []);
    selectedRowIds: Set<string> = new Set();

    public displayedColumns = [...masterJunctionBoxListTableColumn].map(x => x.key);
    ...
    showJunctionMaster: boolean = false;
    private subscription: Subscription;

    constructor(protected appConfig: AppConfig, private bulkDeleteService: BulkDeleteService) { }

    @Input() public set items(value: ReadonlyArray<JunctionBoxListDtoModel>) {
    ...
    protected deleteJunctionBox(id: string) {
        this.delete.emit(id);
    }
    ...
    protected editJunctionBox(id: string) {
        this.edit.emit(id);
    }
    ...
    protected activeInActiveJunction(id: string, isActive: boolean) {
    ...
    checkboxLabel(row?: JunctionBoxListDtoModel): string {
        if (!row) {
    ...
    showDeleteBulk() {
        ...
        .getCheckboxState('junctionMaster')
        .subscribe((show) => {
            this.showJunctionMaster = show;
            this.resetCheckboxes();

            if (this.showJunctionMaster) {
                this.pageSizeOptions = [100];
                this.displayedColumns = ['select', ...masterJunctionBoxListTableColumn.map((x) => x.key)];
                ...
                this.displayedColumns = masterJunctionBoxListTableColumn.map((x) => x.key).filter(x => x != 'select');
                this.cdr.detectChanges();
