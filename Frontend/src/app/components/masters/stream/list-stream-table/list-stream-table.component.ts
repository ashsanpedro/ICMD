...
export class ListStreamTableComponent implements OnInit {
    ...
    selection = new SelectionModel<StreamInfoDtoModel>(true, []);
    ...
    checkboxLabel(row?: StreamInfoDtoModel): string {
        if (!row) {
    ...
    showDeleteBulk() {
        ...
        .subscribe((show) => {
            ...
            if (this.showtfRoom) {
                ...
                this.displayedColumns = ['select', ...masterStreamListTableColumn.map((x) => x.key)];
                ...
                this.displayedColumns = masterStreamListTableColumn.map((x) => x.key).filter(x => x != 'select');
                this.cdr.detectChanges();
