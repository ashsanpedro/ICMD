...
export class ListWorkAreaTableComponent implements OnInit {
    ...
    selection = new SelectionModel<WorkAreaPackInfoDtoModel>(true, []);
    ...
    checkboxLabel(row?: WorkAreaPackInfoDtoModel): string {
        if (!row) {
    ...
    showBulkDelete() {
        ...
        .subscribe((show) => {
            ...
            if (this.showWapMaster) {
                ...
                this.displayedColumns = ['select', ...masterWorkAreaListTableColumn.map((x) => x.key)];
                ...
                this.displayedColumns = masterWorkAreaListTableColumn.map((x) => x.key).filter(x => x !== 'select');
                this.cdr.detectChanges();
