...
export class ListReferenceDocumentTableComponent implements OnInit {
    ...
    selection = new SelectionModel<ReferenceDocumentInfoDtoModel>(true, []);
    ...
    checkboxLabel(row?: ReferenceDocumentInfoDtoModel): string {
        if (!row) {
    ...
    showDeleteBulk() {
        ...
        .getCheckboxState('refDocMaster')
        .subscribe((show) => {
            this.showRefMaster = show;
            this.resetCheckboxes();

            if (this.showRefMaster) {
                this.pageSizeOptions = [100];
                this.displayedColumns = ['select', ...masterReferenceDocumentListTableColumn.map((x) => x.key)];
                ...
                this.displayedColumns = masterReferenceDocumentListTableColumn.map((x) => x.key).filter(x => x != 'select');
                this.cdr.detectChanges();
