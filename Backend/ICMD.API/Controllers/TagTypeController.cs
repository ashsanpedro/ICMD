...
namespace ICMD.API.Controllers
{
    ...
    public class TagTypeController : BaseController
    {
                            ...
                            if (isSuccess)
                            {
                                ...
                                try
                                {
                                    ...
                                    if (isEditImport && editId != Guid.Empty)
                                    {
                                        ...
                                        existingTagType = await _tagTypeService.GetSingleAsync(x => x.Id == editId &&
                                        ...
                                        {
                                            var existingRecordName = await _tagTypeService.GetSingleAsync(x => x.Id != editId &&
                                    ...
                                    {
                                        existingTagType = await _tagTypeService.GetSingleAsync(x => x.Name.ToLower().Trim() == createDto.Name.ToLower().Trim() && !x.IsDeleted);
                                    }
                                    if (message.Count == 0)
                                    {
        ...
        [HttpPost]
        [AuthorizePermission(Operations.Add)]
        public async Task<ImportFileResultDto<ValidationDataDto>> ValidateImportTagType([FromForm] FileUploadModel info)
        {
            ...
            if (info.File != null && info.File.Length > 0)
            {
                    ...
                    && typeHeaders != null)
                {
                    ...
                    var transaction = await _tagTypeService.BeginTransaction();
                                ...
                                try
                                {
                                    ...
                                    if (isEditImport && editId != Guid.Empty)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            {
                                                message.Add("Name is already taken.");
                                                validationData.Changes = GetChanges(existingTagType, createDto);
                                            }
                                        }
                                    }
                                    else
                    ...
                    await _tagTypeService.RollbackTransaction(transaction);
                }
                else
                {
                    return new() { Message = ResponseMessages.GlobalModelValidationMessage };
        ...
        private List<ChangesDto> GetChanges(TagType entity, CreateOrEditTagTypeDto createDto)
        {
